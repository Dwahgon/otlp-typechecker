...
test9 = testTemplate "\\x.(x, 'a')" (TVar "t1" --> (TVar "t1" |<>| TCon "Char"))
test10 = testTemplate "\\y.let f = \\x.x in (f True, f y)" (TVar "t3" --> (TCon "Bool" |<>| TVar "t3"))
test11 = testTemplate "\\x.case x of {True -> 1; False -> 0}" (TCon "Bool" --> TCon "Int")